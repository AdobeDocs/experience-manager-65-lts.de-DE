...
* **FORMS-24689:** Im Turnkey-Upgrade-Modus werden Aktualisierungen am Datenbank-Port nicht über die Benutzeroberfläche ausgewählt und müssen in der Konfigurationsdatei lc_turnkey.xml manuell geändert werden. Wenn der Port nicht korrekt aktualisiert wird, verwendet das System möglicherweise weiterhin den alten Port-Wert.

* **FORMS-24687:** In LTS-Cluster-Bereitstellungen von AEM Forms 6.5.1 auf JBoss EAP 8 können `domain/configuration/domain_oracle.xml` ein doppeltes `<security>`-Tag enthalten, was zu ungültigem XML führt, das den Start des Domain-Controllers verhindert.
