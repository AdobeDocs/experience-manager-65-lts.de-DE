...
>[!IMPORTANT]
>
> **Obligatorischer Hotfix** - Um SNFE-Probleme (SegmentNotFoundException) bei der Offline-Komprimierung bei der Installation von Service Pack 1 zu vermeiden, installieren Sie den Hotfix unter [Bekannte Probleme - Repository-Beschädigung während der Online-Komprimierung](#repository-corruption-during-online-compaction-after-offline-compaction-granite-65146).
...
Weitere Informationen finden Sie unter **Migration von Javax zum Jakarta-Namespace**.
...
* Vorher (JavaScript)    Nach (Jakarta)
...
**Umgebung**
Gilt für: AEM 6.5 LTS (On-Premise)-Kunden, die Service Pack 1 (SP1) installieren. SP1 wird als Schnellstart-JAR bereitgestellt.

**Warum das wichtig ist**
SP1 für AEM 6.5 LTS wird als Schnellstart-JAR und nicht als ZIP-Datei zur Installation über Package Manager bereitgestellt. On-Premise-Kundinnen und -Kunden führen ein Upgrade durch, indem sie die Schnellstart-JAR-Datei ersetzen, entpacken und neu starten. Diese Methode entspricht dem Upgrade-Verfahren von Adobe.
...
* **FORMS-24688:** Beim Einrichten von JBoss EAP 8.0 (AEM Forms 6.5.1 LTS) unter Linux können in Windows erstellte oder bearbeitete Skripte CRLF-Zeilenenden enthalten, was zu Fehlern bei /bin/sh^M: ungültiger Interpreter oder $&#39;\r&#39;: Befehl nicht gefunden führt. Stellen Sie sicher, dass die betroffenen Dateien LF-Zeilenenden verwenden. Dies lässt sich mit Tools wie dos2unix oder einer gleichwertigen Methode erreichen.

* **FORMS-24689:** Im Turnkey-Modus wird die Aktualisierung des Datenbank-Ports bei einer Neuinstallation oder einem Upgrade nicht korrekt angewendet. Im Neuinstallationsmodus können Benutzer nur die Ports auswählen, die auf dem Bildschirm angezeigt werden. Im Upgrade-Modus muss der neue Datenbank-Port manuell in lc_turnkey.xml aktualisiert werden, bevor mit dem Upgrade fortgefahren wird.
...
Bei der Online-Komprimierung kam es zu einer Beschädigung des Repositorys, wenn zuvor eine Offline-Komprimierung auf dem JCR-Repository ausgeführt wurde. In diesem Szenario kann ein `SegmentNotFoundException` (SNFE) auftreten. Dies kann zu einer Beschädigung des Repositorys führen.

Installieren Sie zur Behebung des Problems den Hotfix über Package Manager von [Software Distribution](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/cq-6.5.lts.1-hotfix-GRANITE-65385-1.0.zip). Da der Hotfix ein `oak-segment-tar`-Bundle auf niedriger Ebene enthält, wird die Instanz nach der Installation neu gestartet.

Planen Sie Ausfallzeiten einer Instanz bei deren Anwendung ein. Verwenden Sie für die Offline-Komprimierung die entsprechende [JAR mit Ausführung von Oak](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/oak-run-1.78.1-B006.jar), die auch in Software Distribution verfügbar ist.

>[!NOTE]
>
> Verwenden Sie für alle Oak-ausgeführten Vorgänge das (oben verlinkte) JAR[&#128279;](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/oak-run-1.78.1-B006.jar) Oak-run 1.78.1-B006.
