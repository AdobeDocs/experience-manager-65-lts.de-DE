...
* **FORMS-24688:** Beim Einrichten von JBoss EAP 8.0 unter Linux können unter Windows geänderte Shell-Skripte CRLF-Zeilenenden enthalten, was zu Fehlern von `/bin/sh^M: bad interpreter` oder $`\r`: Befehl nicht gefunden führt. Konvertieren Sie betroffene Dateien mithilfe von dos2unix in LF-Zeilenenden, um dies zu beheben.

* **FORMS-24689:** Im Turnkey-Upgrade-Modus wird die Aktualisierung des Datenbank-Ports in `lc_turnkey.xml` während des Upgrades möglicherweise nicht korrekt angewendet, was dazu führt, dass der Prozess auf den alten Port-Wert verweist.
