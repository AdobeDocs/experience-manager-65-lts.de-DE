---
title: Aktuelle Versionshinweise zu Adobe Experience Manager 6.5 LTS.
...
Bei der Aktivierung der Funktion „Nur SSL“ in AEM-Bereitstellungen gibt es ein bekanntes Problem, das die Verbindung zwischen dem Dispatcher und AEM-Instanzen beeinträchtigt. Nach Aktivierung dieser Funktion können Konsistenzprüfungen fehlschlagen und die Kommunikation zwischen dem Dispatcher und AEM-Instanzen kann unterbrochen werden. Dieses Problem tritt insbesondere auf, wenn Kunden versuchen, eine Verbindung über `https + IP` vom Dispatcher zu AEM-Instanzen herzustellen, und steht im Zusammenhang mit SNI-Validierungsproblemen (Server Name Indication).
